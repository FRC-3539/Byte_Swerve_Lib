...
import org.frcteam3539.Byte_Swerve_Lib.math.MathUtils;
...
public class Trajectory {
    ...
    public Trajectory(Path path, TrajectoryConstraint[] trajectoryConstraints, double sampleDistance,
        ...
        while (distance < path.getLength()) {
            ...
            // If the max ending velocity is lower than the starting velocity we know that
            ...
            if (MathUtils.epsilonEquals(optimalAcceleration, 0.0)) {
                ...
                for (TrajectoryConstraint constraint : trajectoryConstraints) {
                    ...
                    maxEndingAcceleration = Math.min(constraint.getMaxAcceleration(endingState, startingVelocity),
                            maxEndingAcceleration); // TODO: Use endingVelocity instead of startingVelocity
                }
        ...
        for (int i = constrainedPathStates.size() - 1; i >= 0; i--) {
            ...
            if (deltaVelocity < 0.0) {
                ...
                double decelDist = 0.5 * deceleration * Math.pow(decelTime, 2.0)
                        + constrainedState.endingVelocity * decelTime;
                ...
                if (decelDist > constrainedState.length) {
                    ...
                    // we can.
                    double[] roots = MathUtils.quadratic(0.5 * deceleration, constrainedState.endingVelocity,
                            -constrainedState.length);
                    ...
                    constrainedState.startingVelocity = constrainedState.endingVelocity
                            + deceleration * maxAllowableDecelTime;
                }
            }
        }
